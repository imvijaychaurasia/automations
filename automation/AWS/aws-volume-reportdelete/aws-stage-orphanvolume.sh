...
if [ -z "$volume_report" ]; then
  ...
  slack_message=":white_check_mark: AWS Stage - Congratulations! Your Volumes are clean and in excellent shape. There are no orphaned Volumes found in ap-south-1 region to worry about."
  ...
  slack_message=":warning: AWS Stage - CRITICAL ISSUE: We have found orphan volumes are in ap-south-1 region:\n\n$volume_report\n\nProceeding with deletion of the reported volumes."
  SLACK_PAYLOAD="{\"text\": \"$slack_message\"}"
  curl -X POST -H 'Content-type: application/json' --data "$SLACK_PAYLOAD" https://hooks.slack.com/services/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
