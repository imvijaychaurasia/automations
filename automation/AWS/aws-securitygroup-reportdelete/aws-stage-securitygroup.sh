...
if [ -z "$security_group_report" ]; then
  ...
  slack_message=":white_check_mark: AWS Stage - Congratulations! Your Security Groups are clean and in excellent shape. There are no orphaned Security Groups found in ap-south-1 region to worry about."
  ...
  slack_message=":warning: AWS Stage - URGENT ALERT: Unattached Security Groups detected in ap-south-1 Region.:\n\n$security_group_report\n\nProceeding with the deletion of the reported Security Groups."
  SLACK_PAYLOAD="{\"text\": \"$slack_message\"}"
  curl -X POST -H 'Content-type: application/json' --data "$SLACK_PAYLOAD" https://hooks.slack.com/services/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
